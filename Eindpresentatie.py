...
df['geslacht'] = np.where((df['sex'] == 1),'man', 'vrouw')
df = df[['year', 'statefip', 'age', 'geslacht', 'hrswork', 'uhrswork', 'white', 'black', 'hisp', 'othrace', 'annhrs', 'niincwage', 'hrwage', 'realhrwage', 'female', 'Agriculture','miningconstruction','durables','nondurables','Transport','Utilities','Communications','retailtrade','wholesaletrade','finance','SocArtOther','hotelsrestaurants','Medical','Education','professional','publicadmin','sumadj_ind','manager','business','financialop','computer','architect','scientist','socialworker','postseceduc','legaleduc','artist','lawyerphysician','healthcare','healthsupport','protective','foodcare','building','sales','officeadmin','farmer','constructextractinstall','production','transport']]
df.rename(columns = {'building':'Maintenance'}, inplace = True)
df['beroep'] = np.where((df['architect'] == 1),'architect', '0')
df['beroep'] = np.where((df['scientist'] == 1),'scientist', df['beroep'])
df['beroep'] = np.where((df['socialworker'] == 1),'socialworker', df['beroep'])
df['beroep'] = np.where((df['artist'] == 1),'artist', df['beroep'])
df['beroep'] = np.where((df['lawyerphysician'] == 1),'lawyerphysician', df['beroep'])
df['beroep'] = np.where((df['healthcare'] == 1),'healthcare', df['beroep'])
df['beroep'] = np.where((df['sales'] == 1),'sales', df['beroep'])
df['beroep'] = np.where((df['officeadmin'] == 1),'officeadmin', df['beroep'])
df['beroep'] = np.where((df['farmer'] == 1),'farmer', df['beroep'])
df['beroep'] = np.where((df['Agriculture'] == 1),'Agriculture', df['beroep'])
df['beroep'] = np.where((df['miningconstruction'] == 1),'miningconstruction', df['beroep'])
df['beroep'] = np.where((df['durables'] == 1),'durables', df['beroep'])
df['beroep'] = np.where((df['nondurables'] == 1),'nondurables', df['beroep'])
df['beroep'] = np.where((df['Transport'] == 1),'Transport', df['beroep'])
df['beroep'] = np.where((df['Utilities'] == 1),'Utilities', df['beroep'])
df['beroep'] = np.where((df['Communications'] == 1),'Communications', df['beroep'])
df['beroep'] = np.where((df['retailtrade'] == 1),'retailtrade', df['beroep'])
df['beroep'] = np.where((df['wholesaletrade'] == 1),'wholesaletrade', df['beroep'])
df['beroep'] = np.where((df['SocArtOther'] == 1),'SocArtOther', df['beroep'])
df['beroep'] = np.where((df['hotelsrestaurants'] == 1),'hotelsrestaurants', df['beroep'])
df['beroep'] = np.where((df['Medical'] == 1),'Medical', df['beroep'])
df['beroep'] = np.where((df['Education'] == 1),'Education', df['beroep'])
df['beroep'] = np.where((df['professional'] == 1),'professional', df['beroep'])
df['beroep'] = np.where((df['publicadmin'] == 1),'publicadmin', df['beroep'])
df['beroep'] = np.where((df['manager'] == 1),'manager', df['beroep'])
df['beroep'] = np.where((df['business'] == 1),'business', df['beroep'])
df['beroep'] = np.where((df['financialop'] == 1),'financialop', df['beroep'])
df['beroep'] = np.where((df['computer'] == 1),'computer', df['beroep'])
df['beroep'] = np.where((df['postseceduc'] == 1),'postseceduc', df['beroep'])
df['beroep'] = np.where((df['legaleduc'] == 1),'legaleduc', df['beroep'])
df['beroep'] = np.where((df['healthsupport'] == 1),'healthsupport', df['beroep'])
df['beroep'] = np.where((df['protective'] == 1),'protective', df['beroep'])
df['beroep'] = np.where((df['foodcare'] == 1),'foodcare', df['beroep'])
df['beroep'] = np.where((df['Maintenance'] == 1),'Maintenance', df['beroep'])
df['beroep'] = np.where((df['constructextractinstall'] == 1),'constructextractinstall', df['beroep'])
df['beroep'] = np.where((df['production'] == 1),'farproductionmer', df['beroep'])
df['beroep'] = np.where((df['transport'] == 1),'transport', df['beroep'])
df = df[['year', 'statefip', 'age', 'geslacht', 'hrswork', 'uhrswork', 'white', 'black', 'hisp', 'othrace', 'annhrs', 'niincwage', 'hrwage', 'realhrwage','beroep']]
fip = pd.read_csv('us-state-ansi-fips.csv', skipinitialspace = True)
df = pd.merge(df, fip, left_on='statefip',right_on='st', how='left')
